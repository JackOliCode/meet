...
describe('<App /> integration', () => {
  ...
  test('get list of all events when user selects "See all cities"', async () => {
    ...
    await suggestionItems.at(suggestionItems.length - 1).simulate('click'); //The test simulates a click on the last list item (which will always be “See all cities”)
    const allEvents = await getEvents();
    expect(AppWrapper.state('events')).toEqual(allEvents);
    AppWrapper.unmount();
  });
  // - -------------------------- NumberOfEvents int. testing starts here -----------------------------//
  ...
  test('App component updates numberOfEvents state when NumberOfEvents component is used', () => {
    const AppWrapper = mount(<App />);
